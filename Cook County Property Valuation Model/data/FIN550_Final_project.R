#It takes 30 mins to run the entire script
...
# Predict sale price
predict_orig$pred_sale_price <- predict(lasso_model, newx=xpred, s="lambda.min")
prediction <- subset(predict_orig, select = c("pid", "pred_sale_price"))
...
pred_df$pred_sale_price[is.na(pred_df$pred_sale_price)] <- mean(pred_df$pred_sale_price, na.rm = TRUE)
...
response_variable <- "sale_price"

# Prepare training data
x_train <- train_imp[, !colnames(train_imp) %in% response_variable]
